Contributing to Lipcore
=======

We're working hard to make *lipcore* the most powerful JavaScript library for working with lipcoin. Our goal is to have *lipcore* be a library that can be used by anyone interested in lipcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in lipcore that any change must adhere.
...
When possible, lipcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Lipcore methods. For example, calling `JSON.stringify` on a Lipcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'lipcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the lipcore object.

DO:
```javascript
var lipcore = require('../');
var PublicKey = lipcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add lipcoin-project git@github.com:lipcoin-project/lipcore.git
git pull --rebase lipcoin-project master
```
...
```
Finally go to [github.com/lipcoin/lipcore](https://github.com/lipcoin/lipcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of lipcore.

If you have any questions feel free to post them to
[github.com/lipcoin/lipcore/issues](https://github.com/lipcoin/lipcore/issues).
